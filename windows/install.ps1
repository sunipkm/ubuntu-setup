...
if (Test-Path $WSL_RESUME_SRC) {
    ...
    $srcResolved = (Resolve-Path $WSL_RESUME_SRC -ErrorAction SilentlyContinue | Select-Object -ExpandProperty Path)
    $dstResolved = (Resolve-Path $WSL_RESUME      -ErrorAction SilentlyContinue | Select-Object -ExpandProperty Path)
...
if (Test-Path $WSL_BOOTSTRAP_SRC) {
    $srcB = (Resolve-Path $WSL_BOOTSTRAP_SRC -ErrorAction SilentlyContinue | Select-Object -ExpandProperty Path)
    $dstB = (Resolve-Path $WSL_BOOTSTRAP      -ErrorAction SilentlyContinue | Select-Object -ExpandProperty Path)
